...
class Paginator{
	...
	function paginatePlus($ipp,$page,$function,$valFunction)
	{
		...
		if($this->num_pages > 10)
		{
			$this->return = ($this->current_page != 1 And $this->items_total >= 10) ? "<a class=\"paginate icon item\" href=# onclick=\"$function($this->items_per_page,$prev_page,$valFunction)\"><i class=\"left arrow icon\"></i></a> ":"<span class=\"inactive disabled icon item\" href=\"#\"><i class=\"left arrow icon\"></i></span> ";
			...
			if($i==1 Or $i==$this->num_pages Or in_array($i,$this->range))
			{
			$this->return .= ($i == $this->current_page And $page != 'All') ? "<span class=\"current active item\">$i</span> ":"<a class=\"paginate item\" title=\"Go to page $i of $this->num_pages\" href=# onclick=\"$function($this->items_per_page,$i,$valFunction)\">$i</a> ";
			}
			if($this->range[$this->mid_range-1] < $this->num_pages-1 And $i == $this->range[$this->mid_range-1]) $this->return .= " ... ";
			}
			$this->return .= (($this->current_page != $this->num_pages And $this->items_total >= 10) And ($page != 'All')) ? "<a class=\"paginate icon item\" href=# onclick=\"$function($this->items_per_page,$Siguiente_page,$valFunction)\"><i class=\"right arrow icon\"></i></a>\n":"<span class=\"inactive disabled icon item\" href=\"#\"><i class=\"right arrow icon\"></i></span>\n";
			...
			for($i=1;$i<=$this->num_pages;$i++)
			{
			$this->return .= ($i == $this->current_page) ? "<span class=\"current active item\">$i</span>":"<a class=\"paginate item\" href=# onclick=\"$function($this->items_per_page,$i,$valFunction)\">$i</a>";
			}
