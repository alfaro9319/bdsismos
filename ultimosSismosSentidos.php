 	...
 	function tablaSismos($pageSize,$currentPage,$form){
		...
		$n=$result["Count"];
 		for($i=0; $i<$n; $i++){
 			...
 			$html.="<td>".((trim($result["intensidad"][$i])!="")?"<a href='//www.igp.gob.pe/version-anterior/sites/default/files/images/documents/sismos/Boletines_sismicos_".date('Y/',strtotime($result['fecha_local'][$i]." ".$result['hora_local'][$i])).date('dmyHi',strtotime($result['fecha_local'][$i]." ".$result['hora_local'][$i])).".pdf' target='_blank'><img style='height: 20px;' src='https://cdn1.iconfinder.com/data/icons/hawcons/32/699327-icon-55-document-text-48.png'></a>":"")."</td>";
		        $html.="</tr>";
