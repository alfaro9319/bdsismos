<?php
	...
	function listMonth(){
		...
		$sql = "SELECT MONTH(fecha_local) AS mes, COUNT(*) AS cantidad FROM listasismos WHERE YEAR(fecha_local) = ".date("Y")." GROUP BY MONTH(fecha_local) ORDER BY MONTH(fecha_local)";
	...
	function readListSismos($option,$pageSize=10,$currentPage=1,$valOption=""){
		...
		$limitIni=($currentPage-1)*$pageSize;
		if ($option=="All"){
			$sql = "SELECT * FROM listasismos WHERE YEAR(fecha_local) = ".date("Y")."   ORDER BY fecha_local DESC, hora_local DESC";
		}
		elseif ($option=="All-Asc"){
			$sql = "SELECT * FROM listasismos WHERE YEAR(fecha_local) = ".date("Y")." ORDER BY fecha_local ASC, hora_local ASC";
		}
		elseif ($option=="Limit"){
			$sql = "SELECT * FROM listasismos WHERE YEAR(fecha_local) = ".date("Y")." ORDER BY fecha_local DESC, hora_local DESC LIMIT $limitIni,$pageSize";
		}
		elseif ($option=="FilterMonth"){
			$sql = "SELECT * FROM listasismos WHERE YEAR(fecha_local) = ".date("Y")." AND MONTH(fecha_local) IN ($valOption)  ";
			$sql.= "ORDER BY fecha_local DESC, hora_local DESC";
		}
		elseif ($option=="FilterMonth-Asc"){
			$sql = "SELECT * FROM listasismos WHERE YEAR(fecha_local) = ".date("Y")." AND MONTH(fecha_local)= $valOption  ";
